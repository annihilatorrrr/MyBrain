...
import com.mhss.app.widget.tasks.TasksWidgetSettings
import kotlinx.coroutines.launch
import kotlin.math.roundToInt

class TasksWidgetConfigurationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            var backgroundOpacity by remember {
                mutableFloatStateOf(TasksWidgetSettings.DEFAULT_BACKGROUND_OPACITY)
            }
            var settingsLoaded by remember { mutableStateOf(false) }

            LaunchedEffect(glanceId) {
                backgroundOpacity = TasksWidgetSettings.getBackgroundOpacity(
                    this@TasksWidgetConfigurationActivity,
                    glanceId
                )
                settingsLoaded = true
            }
            ...
            MyBrainTheme {
                Surface(modifier = Modifier.fillMaxSize()) {
                    Column(
                        ...
                        Text(
                            text = stringResource(R.string.tasks_widget_settings),
    ...
    private fun save(backgroundOpacity: Float) {
        lifecycleScope.launch {
            TasksWidgetSettings.setBackgroundOpacity(
                this@TasksWidgetConfigurationActivity,
                glanceId,
                backgroundOpacity
            )
            TasksWidget().update(this@TasksWidgetConfigurationActivity, glanceId)
            setResult(
                RESULT_OK,
                Intent().putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
            )
